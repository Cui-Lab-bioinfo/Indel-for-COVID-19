...
class SimpleSampleContainer(object):
    ...
    def growth_rate(self):
        return np.dot(self.samples.growth_rate.T, self.weights)
...
class StreamingSampleContainer(object):
    ...
    def __call__(self, sample):
        ...
        if self._num_samples == 1.0:
            ...
            if hasattr(sample, 'growth_rate'):
                self._growth_rate = sample.growth_rate * sample.weight
                self._growth_rate_sq = np.square(sample.growth_rate) * sample.weight
            ...
            if hasattr(sample, 'growth_rate'):
                self._growth_rate = factor * self._growth_rate + \
                    (sample.growth_rate * sample.weight) / self._num_samples
                self._growth_rate_sq = factor * self._growth_rate_sq + \
                    (np.square(sample.growth_rate) * sample.weight) / self._num_samples
    ...
    @cached_property
    def growth_rate(self):
        return self._normalizer * self._growth_rate
