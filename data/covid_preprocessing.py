...
def _compute_y_gamma(N, genotype, locations, args, phi=None, verbose=True):
    ...
    for r in range(num_regions):
        ...
        # print(f"N_sum {N_sum_r} min_biweekly {args.min_biweekly_samples} densely_sample {densely_sampled}")
        ...
        # if torch.isnan(numerator).all() or torch.isnan(denominator).all():
        #
        if len(numerator) == 0:
            numerator = torch.tensor([0],device="cuda:0")
            denominator = torch.tensor([0],device = "cuda:0")
        denominator = torch.tensor([max(1e-10, den) for den in denominator], device="cuda:0")
        # print(f"numerator {numerator} denominatro {denominator}")
...
def main(args):
    ...
    clade_to_lineage = data["clade_to_lineage"]
    clades = sorted(list(clade_to_lineage.keys()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filename', type=str, default=f'mutrans.{suffix}_new.pt')
    parser.add_argument('--pyrocov-dir', type=str, default='/home/yanhongliang/projects/local-bvas/results/')
